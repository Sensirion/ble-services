...
const ServiceInfoContent = ({content}: ServiceInfoProps) => {
    const isStandard = content["ble-sig-reference"];
    return (
        <div className="service_info_content">
            <div>
                <b>Service UUID:</b> {content.uuid}
            </div>
            <div>
                <b>Service Type:</b> {isStandard ?
                <span>Standard <ReferenceLink link={content["ble-sig-reference"]}/></span> : 'Sensirion internal'}
            </div>
...
const PropertyIcons = ({properties}: { properties: BLECharacteristicsProperties }) => {
    return (
        <div className="property-icons">
            ...
            <FontAwesomeIcon icon={faPencilAlt} title="Write"
                             className={properties.includes("write") ? '' : 'inactive'}/>
